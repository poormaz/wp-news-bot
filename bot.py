...
USER_AGENT = os.getenv(
    "USER_AGENT",
    "Mozilla/5.0 (WPNewsBot/1.0; +https://example.com)"
).strip()
...
# RankMath updater (optional) - if empty => disabled
...
DEDUP_SIM_THRESHOLD = float(os.getenv("DEDUP_SIM_THRESHOLD", "0.80").strip() or "0.80")
...
# Behavior switches requested by you:
# 1) Article can be short -> OK (no "too short" guard).
# 2) Any failure => mark failed and move to next source in rotation (no crash).
FAIL_AND_MOVE_TO_NEXT_SOURCE = True
...
def safe_env_report():
    keys = [
        ...
        "PEXELS_API_KEY",
        "ROTATION_SOURCES",
        "FEED_ENTRIES_LIMIT", "MAX_POSTS_PER_RUN",
        "DEDUP_WINDOW_HOURS", "DEDUP_SIM_THRESHOLD", "DEDUP_WP_CHECK",
        "USE_SOURCE_PAGE_TEXT", "SOURCE_TEXT_MAX_CHARS",
    ]
    print("\nENV CHECK (safe):\n")
    for k in keys:
        v = os.getenv(k, "")
...
def is_duplicate_by_db(title_en: str) -> tuple[bool, str]:
    ...
    best = 0.0
    best_src = ""
    for src, tnorm, _pts in candidates:
...
def load_sources() -> list[dict]:
    if not os.path.exists(SOURCES_FILE):
        die(f"Missing SOURCES_FILE: {SOURCES_FILE}")
    ...
    if not isinstance(sources, list) or not sources:
        die("sources.yaml is empty. Expected: sources: - name: ... feed: ...")
...
def fetch_source_text_excerpt(source_url: str, max_chars: int = SOURCE_TEXT_MAX_CHARS) -> str:
    ...
    # Remove common boilerplate snippets
    text = re.sub(r"(?i)\b(read more|continue reading|see more|learn more)\b", " ", text)
    text = re.sub(r"\s+", " ", text).strip()
...
- Keep it concise and readable. If page_text is short, write a short but useful article without guessing.
...
""".strip()
    ...
    try:
        ...
        print("WARN: structured output failed; fallback to json_object. Error:", repr(e))
        resp = client.chat.completions.create(
            model=OPENAI_MODEL,
            temperature=OPENAI_TEMPERATURE,
            messages=[
    ...
    # Remove accidental source URL if the model leaked it
    if source_url:
        html_out = html_out.replace(source_url, "").strip()

    # IMPORTANT: no "too short" failure anymore
    data["content_html_fa"] = html_out

    if not data["title_fa"] or not data["content_html_fa"]:
...
def wp_check_me():
    ...
    if r.status_code >= 400:
        print("WP ME error body (first 300):", (r.text or "")[:300])
    r.raise_for_status()
    return r.json()
...
def wp_upload_media(image_bytes: bytes, filename: str, mime_type: str, alt_text: str) -> dict:
    ...
    if r.status_code >= 400:
        print("WP MEDIA ERROR (first 400):", (r.text or "")[:400])
    r.raise_for_status()
    media = r.json()

    # Patch alt_text (optional)
    if alt_text:
        patch = requests.post(
...
def create_wp_post(title: str, content_html: str, categories: list[int], featured_media_id: int | None = None) -> int:
    ...
    if r.status_code >= 400:
        print("WP POST ERROR (first 500):", (r.text or "")[:500])
    r.raise_for_status()
...
def wp_search_similar_posts(title_en: str) -> tuple[bool, str]:
    ...
    if r.status_code >= 400:
        print("WP SEARCH error (first 300):", (r.text or "")[:300])
        return False, ""
    ...
    best_title = ""
    for p in posts:
        rendered = clean_text(((p.get("title") or {}).get("rendered") or "").strip())
...
def pexels_search_photo(query: str) -> dict | None:
    ...
    if r.status_code >= 400:
        print("PEXELS ERROR (first 300):", (r.text or "")[:300])
        return None
    data = r.json() or {}
...
def pexels_attribution_html(photo: dict) -> str:
    if not photo:
        return ""
    photographer = clean_text(photo.get("photographer") or "")
...
def build_wp_content(
    ...
    footer = (
        ...
        f'<a href="{source_url}" target="_blank" rel="nofollow noopener">لینک</a><br/>'
        f"<strong>زمان انتشار منبع:</strong> {clean_text(nice_date)}</p>"
    )
...
def pick_categories(title_en: str, snippet_en: str) -> list[int]:
    ...
    if CAT_HARDWARE and any(k in text for k in ["gpu", "rtx", "radeon", "cpu", "intel", "amd", "nvidia", "laptop", "ssd", "ram", "motherboard", "dlss", "fsr"]):
        if CAT_ALL:
            return [CAT_ALL, CAT_HARDWARE]
        return [CAT_HARDWARE]

    if CAT_GAMING and any(k in text for k in ["game", "gaming", "steam", "ps5", "xbox", "nintendo", "dlc", "trailer"]):
        if CAT_ALL:
            return [CAT_ALL, CAT_GAMING]
        return [CAT_GAMING]
    ...
    for _ in range(max(1, MAX_POSTS_PER_RUN)):
        ...
        while True:
            ...
            try:
                ...
                # Build page_text (even if short => OK)
                page_text = ""
                if USE_SOURCE_PAGE_TEXT:
                    page_text = fetch_source_text_excerpt(url)
                    print("page_text chars:", len(page_text))

                # Generate Persian article (even if short => OK)
                gen = openai_generate_fa_article(
                    title_en, snippet_en, source_name, url, page_text=page_text
                )

                # Featured image handling (best-effort)
                ...
                if SET_FEATURED_IMAGE:
                    ...
                    if img_url:
                        ...
                        print("No image found (source + pex
