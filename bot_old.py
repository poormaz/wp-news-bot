...
def make_latin_slug(*parts: str, max_len: int = 85) -> str:
    """Build a readable Latin slug/filename from English parts."""
...
def make_persian_slug(*parts: str, max_len: int = 105) -> str:
    """Build a Persian/Latin mixed WordPress slug, e.g. زیرنویس-فارسی-007-first-light."""
    base = " ".join(clean_text(p) for p in parts if clean_text(p))
    if not base:
        return ""

    base = base.lower()
    # Keep Persian/Arabic letters, Latin letters, digits, spaces and hyphens.
    base = re.sub(r"[^0-9a-z\u0600-\u06FF\s-]", " ", base, flags=re.UNICODE)
    base = re.sub(r"\s+", "-", base).strip("-")
    base = re.sub(r"-{2,}", "-", base)
    if not base:
        return ""
...
def make_wp_slug(*parts: str, max_len: int = 105) -> str:
    """Use Latin slugs when enabled, otherwise Persian/Latin mixed slugs for Persian SEO."""
    if SEO_USE_LATIN_SLUG:
        return make_latin_slug(*parts, max_len=min(max_len, 85))
    return make_persian_slug(*parts, max_len=max_len)
...
def process_manual_links_if_any() -> bool:
    ...
    try:
        for url in urls:
            ...
            post_slug = make_wp_slug(gen.get("focus_keyword_fa", ""), gen.get("title_fa", ""), gen.get("slug_en", ""), title_en) or f"manual-{url_hash(url)[:12]}"
...
def create_wp_post(
    ...
    if slug:
        payload["slug"] = make_wp_slug(slug)
    if excerpt:
    ...
    for _ in range(max(1, MAX_POSTS_PER_RUN)):
        ...
        while True:
            ...
            try:
                ...
                post_slug = make_wp_slug(gen.get("focus_keyword_fa", ""), gen.get("title_fa", ""), gen.get("slug_en", ""), title_en) or f"news-{item_id[:12]}"
                image_alt = gen.get("image_alt_fa") or gen["title_fa"]
                tag_ids = wp_get_or_create_tag_ids(gen.get("tags_fa", []))
                related_links = wp_search_related_posts(title_en, gen.get("focus_keyword_fa", ""))
