...
def process_manual_links_if_any() -> bool:
    ...
    try:
        for url in urls:
            ...
            # One source request provides a real title, the article text, image and date.
...
def fetch_source_page_data(source_url: str) -> dict:
    ...
    try:
        ...
        return {"title": "", "text": "", "image_url": None, "published_at": ""}
...
def build_wp_content(
    ...
    footer_inner = (
        f"<strong>منبع:</strong> {source_name_safe} — "
        f'<a href="{source_url_safe}" target="_blank" rel="nofollow noopener">لینک</a>'
