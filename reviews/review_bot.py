...
def _editorial_points_for_card(card: dict, evidence_by_id: dict) -> tuple[list[dict], list[dict]]:
    positives = []
    negatives = []
    ...
    for ref_id in card.get("supported_refs", []) or []:
        ...
        if section == "positives":
            ...
            # برای کارت فنی، جهت‌گیری از خود کارت می‌آید.
            if card.get("trend_fa") == "منفی":
                negatives.append(item)
            else:
                positives.append(item)
...
def _build_template_editorial_article_preview(dossier: dict) -> dict:
    ...
    for card in scorecards:
        ...
        positives, negatives = _editorial_points_for_card(card, evidence_by_id)
    ...
    return {
        "status": "preview_only_template_grounded",
        "wordpress_post_created": False,
        "title_fa": title_fa,
        "excerpt_fa": excerpt_fa,
        "markdown": "\n".join(markdown).strip() + "\n",
        ...
        "review_note_fa": "این متن صرفاً پیش‌نمایش است و هیچ پستی در وردپرس ایجاد یا منتشر نشده است.",
        "writing_mode": "template_grounded_no_freeform_claims",
    }
