...
def build_poormaz_assessment(client: OpenAI, dossier: dict) -> dict:
    """
    فقط یک کارت امتیاز ساختاریافته می‌سازد. هنوز مقاله یا پست وردپرس
    نمی‌نویسد. هر دسته باید به شناسه‌ی شواهد تأییدشده وصل باشد.
    """
    ...
    prompt = f"""
You are selecting a structured category scorecard for a Persian game-review aggregation.

Game: {dossier.get("game", "")}
Platform: {dossier.get("platform", "")}
Metacritic score (0-10): {score_info["metacritic_score_10"]}
Selected-review average (0-10): {score_info["selected_review_average_10"]}
Deterministic overall score (0-10): {score_info["overall_score_10"]}

Use ONLY the verified evidence objects below.
Do not invent a category, reason, feature, criticism, or technical claim.

Return exactly one JSON object:
{{
  "scorecard": [
    {{
      "key": "gameplay | world_design | story | technical | visuals",
      "label_fa": "Persian category label",
      "score_10": 0.0,
      "supported_refs": ["S1:positives:1"]
    }}
  ]
}}

Rules:
- Include only categories that have directly relevant evidence.
- Every category MUST cite one or more supported_refs from the evidence list.
- Do not include "visuals" without direct visual or graphical evidence.
- score_10 must be a number from 0.0 to 10.0.
- A category score is an editorial synthesis, but it must match the direction
  and strength of its cited evidence.
- No overall score, prose summary, quote, or extra key is allowed.
...
""".strip()
    ...
    for item in raw.get("scorecard", []) if isinstance(raw, dict) else []:
        if not isinstance(item, dict):
            continue
        ...
        score = as_score_10(item.get("score_10"))

        if score is None:
            continue

        refs = item.get("supported_refs", [])
        ...
        for ref_id in refs:
            ref_id = clean_text(str(ref_id))

            if ref_id in evidence_index and ref_id not in valid_refs:
