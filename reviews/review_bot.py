...
def classify_evidence_rule_based(item: dict) -> tuple[str | None, str | None]:
    """
    دسته‌بندی کاملاً قانون‌محورِ شواهد تأییدشده.
    این مرحله دیگر به مدل زبانی متکی نیست، تا یک «سفر جذاب» به‌اشتباه
    امتیاز ارزش خرید نگیرد.
    """
    ...
    for key in CATEGORY_PRIORITY:
        if _matches_any(evidence, CATEGORY_PATTERNS[key]):
            category = key
            break
...
def build_article_preview(client: OpenAI, dossier: dict) -> dict:
    """
    نسخه‌ی تحریریه‌ای: با مدل زبانی متن طبیعی‌تر می‌سازد، اما تنها از
    fact pack محدود استفاده می‌کند و هر پاراگراف باید شناسه‌ی پشتیبان داشته باشد.
    """
    fact_pack = _article_fact_pack(dossier)
    allowed_supports = _allowed_article_supports(fact_pack)
    scorecard_keys = [item["key"] for item in fact_pack.get("scorecards", [])]

    if not fact_pack.get("evidence") or not scorecard_keys:
        return _fallback_article_preview(dossier)

    prompt = f"""
You are a Persian-language games editor writing a natural, professional, SEO-friendly article preview.

Use ONLY the FACT PACK below. Do not use outside knowledge. Do not add game genre, developer, price,
release claims, platforms, awards, technical details, causes, recommendations, or value-for-money claims
unless they are explicitly in the FACT PACK. Never use direct quotes. Never use Markdown links in text.

Every prose block MUST contain a non-empty supports array. Supports may contain only: META,
evidence IDs (such as S2:positives:1), or scorecard IDs (such as C:gameplay).
Do not mention a scorecard category that is not present in ALLOWED SCORECARD KEYS.
For categories marked "برداشت اولیه", explicitly frame the conclusion as preliminary or limited.
Keep wording careful: "منابع بررسی‌شده", "شواهد فعلی", and "در این پرونده" are preferred.

Return exactly one JSON object:
{{
  "title_fa": "Persian SEO title, 45-75 characters, factual and non-clickbait",
  "excerpt_fa": "Persian meta description, 120-160 characters, factual",
  "lead": {{"text_fa": "70-110 Persian words", "supports": ["META", "C:..."]}},
  "sections": [
    {{"key": "one allowed category key", "text_fa": "55-95 Persian words", "supports": ["C:...", "S..."]}}
  ],
  "strengths": [
    {{"text_fa": "One concise Persian bullet", "supports": ["S..."]}}
  ],
  "cautions": [
    {{"text_fa": "One concise Persian bullet", "supports": ["S..."]}}
  ],
  "technical": {{"text_fa": "45-80 Persian words", "supports": ["C:technical", "S..."]}} or null,
  "conclusion": {{"text_fa": "70-110 Persian words", "supports": ["META", "C:..."]}}
}}

FACT PACK:
{json.dumps(fact_pack, ensure_ascii=False, indent=2)}

ALLOWED SCORECARD KEYS:
{json.dumps(scorecard_keys, ensure_ascii=False)}
""".strip()

    try:
        raw = ask_openai_json(client, prompt, max_tokens=1800)
    except Exception as exc:
        print(f"Article preview writer failed; using rule-based fallback: {repr(exc)}")
        return _fallback_article_preview(dossier)

    title_fa = _clean_article_text(raw.get("title_fa"), min_words=4, max_words=14)
    excerpt_fa = _clean_article_text(raw.get("excerpt_fa"), min_words=10, max_words=34)
    lead = _normalize_article_block(raw.get("lead"), allowed_supports, 35, 125)
    conclusion = _normalize_article_block(raw.get("conclusion"), allowed_supports, 35, 125)

    if not title_fa or not excerpt_fa or not lead or not conclusion:
        print("Article preview writer returned incomplete content; using rule-based fallback.")
        return _fallback_article_preview(dossier)

    scorecard_by_key = {item["key"]: item for item in fact_pack.get("scorecards", [])}
    sections = []
    used_keys = set()
    ...
    for item in raw.get("sections", []) or []:
        ...
        if key not in scorecard_by_key or key in used_keys:
            continue

        block = _normalize_article_block(item, allowed_supports, 24, 110)
        if not block:
            continue

        # هر بخش باید دست‌کم به کارت خود یا یکی از شواهد همان کارت وصل باشد.
        card_id = scorecard_by_key[key]["id"]
        allowed_for_category = set(scorecard_by_key[key].get("supported_refs", [])) | {card_id}
        if not any(ref in allowed_for_category for ref in block["supports"]):
            continue

        sections.append({"key": key, **block})
        used_keys.add(key)

    if not sections:
        print("Article preview writer returned no valid analysis sections; using rule-based fallback.")
        return _fallback_article_preview(dossier)

    def normalize_bullets(values, max_items: int) -> list[dict]:
        items = []
        for value in values or []:
            block = _normalize_article_block(value, allowed_supports, 5, 34)
            if not block:
                continue
            items.append(block)
    ...
    strengths = normalize_bullets(raw.get("strengths"), 4)
    cautions = normalize_bullets(raw.get("cautions"), 4)
    technical = _normalize_article_block(raw.get("technical"), allowed_supports, 18, 95)

    # بخش فنی فقط با کارت یا شاهد فنی معتبر چاپ می‌شود.
    technical_card = scorecard_by_key.get("technical")
    if technical and technical_card:
        allowed_technical = set(technical_card.get("supported_refs", [])) | {technical_card["id"]}
        if not any(ref in allowed_technical for ref in technical["supports"]):
            technical = None
    elif technical:
        technical = None

    game = fact_pack["meta"]["game"]
    scorecard = fact_pack.get("scorecards", [])
    review_sources = dossier.get("review_sources", []) or []
    ...
    markdown = [
        ...
        "",
        "## نتیجه در یک نگاه",
        f"- **امتیاز Poormaz:** {_format_score_10(fact_pack['meta'].get('poormaz_score_10'))}",
    ]

    metascore = fact_pack["meta"].get("metacritic_score_100")
    count = fact_pack["meta"].get("critic_review_count")
    if metascore is not None:
        count_part = f" بر پایه‌ی {count} نقد منتقدان" if count is not None else ""
        markdown.append(f"- **متاکریتیک:** {metascore}/100{count_part}")
    if fact_pack["meta"].get("platform"):
        markdown.append(f"- **پلتفرم پرونده:** {fact_pack['meta']['platform']}")

    markdown.extend([
        "",
        "## نگاه کلی به نقدها",
        lead["text_fa"] + _render_inline_citations(lead["supports"], fact_pack),
        "",
        "## کارت امتیاز Poormaz",
        "",
    ...
    for category in scorecard:
        markdown.append(
            f"| {category['label_fa']} | {_format_score_10(category.get('score_10'))} | "
            f"{category.get('trend_fa', 'نامشخص')} | {category.get('confidence_fa', 'محدود')} |"
        )

    for section in sections:
        category = scorecard_by_key[section["key"]]
        markdown.extend([
            "",
            f"## {category['label_fa']}",
            section["text_fa"] + _render_inline_citations(section["supports"], fact_pack),
        ])

    markdown.extend(["", "## نکات مثبت ثبت‌شده"])
    if strengths:
        for item in strengths:
            markdown.append(
                "- " + item["text_fa"] + _render_inline_citations(item["supports"], fact_pack)
            )
    else:
        markdown.append("- در داده‌های تأییدشده، نکته‌ی کوتاه و مستقل کافی برای این بخش ثبت نشده است.")
    ...
    if cautions:
        ...
        markdown.append("- در داده‌های تأییدشده، نکته‌ی احتیاطی مستقل کافی برای این بخش ثبت نشده است.")

    if technical:
        markdown.extend([
            "",
            "## وضعیت فنی و عملکرد",
            technical["text_fa"] + _render_inline_citations(technical["supports"], fact_pack),
        ])

    markdown.extend([
        "",
        "## جمع‌بندی Poormaz",
        conclusion["text_fa"] + _render_inline_citations(conclusion["supports"], fact_pack),
        "",
        "## منابع بررسی‌شده",
    ])
    ...
    return {
        "status": "preview_only_ai_grounded",
        "wordpress_post_created": False,
        "title_fa": title_fa,
        "excerpt_fa": excerpt_fa,
        "markdown": "\n".join(markdown).strip() + "\n",
        ...
        "review_note_fa": "این متن صرفاً پیش‌نمایش است و هیچ پستی در وردپرس ایجاد یا منتشر نشده است.",
        "writing_mode": "ai_grounded_with_support_ids",
    }
