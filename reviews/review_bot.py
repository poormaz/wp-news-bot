...
def category_score_from_evidence(
    ...
    """
    امتیاز دسته‌ای را به‌صورت قطعی از جهت شواهد می‌سازد، نه از حدس مدل.
    مدل فقط مشخص می‌کند هر شاهد مربوط به کدام دسته و با چه جهت‌گیری است.
    """
    if overall_score is None or not evidence_signals:
        return None, "نامشخص", "محدود"
    ...
    # با یک شاهد، تغییر امتیاز بسیار محدود است؛ هرچه پوشش شواهد بیشتر شود
    # کارت دسته‌ای اجازه دارد کمی از امتیاز کلی دورتر شود.
    coverage_factor = {
        1: 0.40,
        2: 0.70,
    }.get(evidence_count, 1.00)
...
def build_poormaz_assessment(client: OpenAI, dossier: dict) -> dict:
    """
    کارت امتیاز را در دو لایه می‌سازد:
    1) مدل فقط شواهد تأییدشده را دسته‌بندی و جهت‌گیری‌شان را تعیین می‌کند.
    2) امتیاز هر دسته با یک فرمول قطعی، نزدیک به امتیاز کلی محاسبه می‌شود.

    این تابع هنوز مقاله یا پست وردپرس نمی‌سازد.
    """
        ...
        evidence_payload.append(
            {
                ...
                "site": item["site_name"],
                "kind": item["section"],
                "point_fa": item["point_fa"],
                "evidence_en": item["evidence_en"],
            }
        )

    prompt = f"""
You are categorizing verified evidence for a Persian game-review scorecard.

Game: {dossier.get("game", "")}
Platform: {dossier.get("platform", "")}

Use ONLY the verified evidence objects below.
Do not create scores. Do not write a prose summary. Do not invent features,
criticisms, or categories.

Return exactly one JSON object:
{{
  "categories": [
    {{
      "key": "gameplay | world_design | story | technical | visuals | value",
      "evidence": [
        {{
          "ref_id": "S1:positives:1",
...
Rules:
- Include a category only when a cited evidence item directly supports it.
- "gameplay" covers combat, controls, systems, encounters, puzzles, activities,
  progression, and player mechanics.
- "world_design" covers exploration, level/world structure, traversal,
  environmental interaction, and open-world design.
- "story" covers narrative, characters, dialogue, pacing, writing, and quests.
- "technical" covers frame rate, performance, optimization, bugs, UI,
  stability, and technical polish.
- "visuals" requires direct evidence about graphics, art direction, animation,
  image quality, lighting, or visual presentation.
- "value" requires direct evidence about price, length, replayability,
  amount of content relative to cost, or monetization.
- Each evidence ref may appear in at most ONE category.
- Use "positive" for praise, "negative" for criticism, "mixed" for explicitly
  mixed feedback, and "neutral" only for factual technical observations.
- Do not include a category with no evidence.
- No score, label, quote, extra key, or explanation is allowed.
...
""".strip()
    ...
    for item in raw.get("categories", []) if isinstance(raw, dict) else []:
        if not isinstance(item, dict):
            continue

        key = clean_text(str(item.get("key") or "")).lower()
        ...
        for evidence_item in raw_evidence:
            ...
            ref_id = clean_text(str(evidence_item.get("ref_id") or ""))
            sentiment = clean_text(
                str(evidence_item.get("sentiment") or "")
            ).lower()

            if (
                ref_id not in evidence_index
                or ref_id in used_refs_global
                or sentiment not in allowed_sentiments
            ):
                continue

            source_evidence = evidence_index[ref_id]

            signals.append(
                {
                    "ref_id": ref_id,
                    "sentiment": sentiment,
                    "site_name": source_evidence["site_name"],
                }
            )
...
def build_article_preview(dossier: dict) -> dict:
    """
    پیش‌نمایش مقاله را کاملاً از امتیازها و نکات تأییدشده می‌سازد.
    این مرحله از مدل برای نگارش آزاد استفاده نمی‌کند تا متن به ادعاهای
    خارج از پرونده منحرف نشود. خروجی هنوز برای بازبینی انسانی است.
    """
    ...
    intro_bits = []
    if platform:
        intro_bits.append(f"نسخه‌ی بررسی‌شده: {platform}")
    if release_date:
        intro_bits.append(f"تاریخ عرضه در پرونده: {release_date}")
    if metascore_100 is not None:
        if critic_count is not None:
            intro_bits.append(
                f"متاکریتیک: {metascore_100} از ۱۰۰ بر پایه‌ی {critic_count} نقد"
            )
        else:
            intro_bits.append(f"متاکریتیک: {metascore_100} از ۱۰۰")
    if overall_score is not None:
        intro_bits.append(f"امتیاز تجمیعی Poormaz: {_format_score_10(overall_score)}")

    excerpt_fa = (
        f"جمع‌بندی Poormaz از نقدهای منتخب {game}: "
        + " | ".join(intro_bits[:3])
        if intro_bits
        else f"جمع‌بندی Poormaz از نقدهای منتخب {game}."
    )

    markdown = []
    markdown.append(f"# {title_fa}")
    markdown.append("")
    markdown.append(
        "> این متن یک پیش‌نمایش تحریریه‌ای است که فقط از نمره‌ها و "
        "نکات تأییدشده‌ی منابع انتخاب‌شده ساخته شده است. هنوز برای وردپرس منتشر نشده است."
    )
    markdown.append("")
    markdown.append("## نتیجه در یک نگاه")
    ...
    if metascore_100 is not None:
        if critic_count is not None:
            markdown.append(
                f"- **متاکریتیک:** {metascore_100}/100 بر پایه‌ی {critic_count} نقد منتقدان"
            )
        else:
            markdown.append(f"- **متاکریتیک:** {metascore_100}/100")
    if platform:
    ...
    markdown.append("")
    markdown.append("## کارت امتیاز Poormaz")
    markdown.append("")

    if scorecard:
        markdown.append("| بخش | امتیاز | جهت‌گیری | پوشش شواهد |")
        markdown.append("|---|---:|---|---|")
        for category in scorecard:
            markdown.append(
                "| "
    ...
    markdown.append("")
    markdown.append("## نقاطی که منابع بررسی‌شده تحسین کرده‌اند")
    markdown.extend(_render_article_points(positives))

    markdown.append("")
    markdown.append("## نکات احتیاطی منتقدان")
    markdown.extend(_render_article_points(negatives))

    markdown.append("")
    markdown.append("## وضعیت فنی و عملکرد")
    markdown.extend(_render_article_points(technical_notes))

    markdown.append("")
    markdown.append("## جمع‌بندی Poormaz")
    ...
    if overall_score is not None:
        ...
        markdown.append(
            "برای ساخت یک جمع‌بندی عددی، داده‌ی کافی از نمره‌ها در دسترس نبود."
        )

    markdown.extend(_scorecard_summary_sentences(scorecard))

    limited_categories = [
        category.get("label_fa")
        for category in scorecard
        if category.get("confidence_fa") == "محدود"
    ]

    if limited_categories:
        markdown.append(
            "برچسب «محدود» یعنی این بخش فعلاً بر پایه‌ی تعداد کمی شاهد مستقیم "
            "یا یک منبع ساخته شده و نباید هم‌وزن اجماع گسترده‌ی منتقدان تلقی شود: "
            + "، ".join(str(item) for item in limited_categories if item)
            + "."
        )

    markdown.append("")
    markdown.append("## منابع بررسی‌شده")
    ...
    markdown.append("---")
    markdown.append(
        "یادداشت تحریریه: این پیش‌نمایش به‌صورت خودکار تولید شده و پیش از انتشار، "
        "برای لحن، سئو، تصویر شاخص و جزئیات نهایی باید بازبینی شود."
    )
    ...
    return {
        ...
        "review_note_fa": (
            "این متن صرفاً پیش‌نمایش است و هیچ پستی در وردپرس ایجاد یا منتشر نشده است."
        ),
    }
...
def process_review_job(client: OpenAI, item: dict):
    ...
    dossier["article_preview"] = build_article_preview(dossier)
